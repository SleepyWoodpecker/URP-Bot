...
class Client:
    def __init__(
        self, akey=None, pkey=None, host=None, code=None, response=None, keyfile=None
    ):
        if keyfile:
    ...
    def __repr__(self) -> str:
        return (
            ...
            + ",".join(
                [
                    (self.__dict__[i] or "") and (i + "=" + self.__dict__[i])
                    for i in ["akey", "pkey", "host"]
                ]
            )
            + ")"
        )
    ...
    def export_response(self):
        if self.host and ("host" not in self.info or not self.info["host"]):
            self.info["host"] = self.host
        with open("duo_response.json", "w") as f:
    ...
    def generate_signature(self, method, path, time, data):
        ...
        auth = "Basic " + base64.b64encode(
            (self.pkey + ":" + base64.b64encode(signature).decode("ascii")).encode(
                "ascii"
            )
        ).decode("ascii")
        return auth
    ...
    def get_transactions(self):
        ...
        r = requests.get(
            ...
            params=data,
            headers={"Authorization": signature, "x-duo-date": time, "host": self.host},
        )
...
def get_duo_approval() -> None:
    ...
    if pathlib.Path("duo_response.json").is_file() and key_exists:
        c.import_response("duo_response.json")
        if code:
            c.read_code(code)
